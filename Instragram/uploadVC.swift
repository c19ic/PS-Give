...
class uploadVC: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func alignment() {
        ...
        linkText.frame = CGRect(x: titleTxt.frame.origin.x, y: picImg.frame.origin.y + picImg.frame.size.height + 10, width: picImg.frame.size.width, height: 40)
    }
    ...
    @IBAction func publishBtn_clicked(_ sender: AnyObject) {
        ...
        if (linkText.text?.isEmpty)!{
            let alert = UIAlertController(title: "PLEASE", message: "fill the link field", preferredStyle: UIAlertControllerStyle.alert)
            let ok = UIAlertAction(title: "OK", style: UIAlertActionStyle.cancel, handler: nil)
            alert.addAction(ok)
            self.present(alert, animated: true, completion: nil)

        }else{
            object["link"] = linkText.text?.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
        }
